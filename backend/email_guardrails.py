...
def _assert_safe_email(subject, html):
    ...
    for url in scan.urls:
        ...
        if not low.startswith("https://"):
            raise ValueError("Email links/assets must be absolute https (G3)")
        host = urlparse(low).hostname or ""
        if not _host_ok(host) or urlparse(low).username is not None:
            raise ValueError("Shortened, numeric-host or credential-bearing URL (G3)")
    for href, text in scan.anchors:
        ...
        for match in _HOSTISH.finditer(text):
            if not _same_site(match.group(1).lower(), real):
                raise ValueError("Anchor text differs from link host (G3)")